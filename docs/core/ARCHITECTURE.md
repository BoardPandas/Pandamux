...
PandaMUX is a standard three-process Electron application (main, renderer, preload) plus two external entry points, the CLI and the named pipe, that let terminal-based tools drive the running app without an MCP server.
