...
| **V2 protocol (pipe protocol)** | The JSON-RPC protocol over `\\.\pipe\pandamux` used by the CLI, agents, and the orchestrator. It covers `system.*`, `workspace.*`, `pane.*`, `layout.grid`, `surface.*`, `markdown.*`, `diff.*`, `notification.*`, `sidebar.*`, `agent.*`, `clipboard.*`, `ssh.*`, `window.*`, `config.*`, and `theme.*`. The request has a reserved `token` field, but the current server does not validate it, and `hook.event` is not implemented ([CLAUDE.md](../CLAUDE.md)). |
