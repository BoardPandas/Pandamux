...
Every V2 request includes a `token` field read from `PANDAMUX_PIPE_TOKEN` (trimmed, empty string if unset) (main.rs:1109-1113). The current server does not validate that field, so it is reserved protocol data rather than an authentication guarantee; see [Named Pipe Control Plane](../features/NAMED_PIPE_IPC.md#v2-json-rpc-protocol).
