...
Date: 2026-07-05
Status: Approved direction (fully native Rust, Warp/Zed-style). This document is the master plan. **Phase 1 (pnpm migration) is COMPLETE; Phase 2 de-risk spike is COMPLETE; Phase 3 foundation is IN PROGRESS with the Rust workspace, core state, native pipe server, CLI, PTY session foundation, terminal I/O, Rust CI, pane zoom, UI shell projection, feature-gated Iced shell view, app runtime shell wiring, live PTY-backed Iced snapshots, periodic Iced refresh, shell smoke coverage, and multi-pane/multi-tab Iced shell controls landed.**
