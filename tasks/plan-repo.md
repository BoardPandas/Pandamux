...
> **Historical implementation record:** The native Rust rewrite and release phases are complete. Use `AGENTS.md`, `CLAUDE.md`, `CHANGELOG.md`, and the maintained `docs/` wiki for current behavior. This file remains authoritative only for historical rationale, recorded implementation lessons, and the UI design specification in Section 12. Earlier status lines are intentionally preserved as chronology unless explicitly marked superseded.

Date: 2026-07-05
Status: Historical record. Phases 1 through 7 are complete; signed native releases have shipped since v0.37. Later product work is tracked in `CHANGELOG.md` and task-specific plans, not in this rewrite plan.
...
| Packaging/updates | **cargo-packager** (NSIS) + **winresource** (build.rs) | Final Phase 7 decision: GitHub Releases carries one signed `Setup.exe`; the in-app updater reads the Releases API directly, so no Velopack feed is required |
...
### Phase 5: Peripheral UI and integrations parity (COMPLETE)
...
- [x] Claude Code integration (0.32.0, later removed in 0.35.6): this phase originally added `claude_context` startup injection and plugin installation. Version 0.35.6 removed all writes to `~/.claude`; the orchestrator remains bundled for manual installation.
...
### Phase 7: Ship
Status: **COMPLETE.** The code and infrastructure landed in 0.35.0, signed releases have published from the tag workflow since v0.37, the public site describes the native app, and the developer wiki was regenerated for the Rust workspace. Historical external gates and first-release notes below describe the state at the original Phase 7 boundary.
