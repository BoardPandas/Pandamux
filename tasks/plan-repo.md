...
Updated: 2026-07-06 (Phase 3 foundation slice): added the production Cargo workspace, core split tree and backend-owned state, native tokio pipe server, Rust CLI parity for system/workspace/layout/pane/surface basics, a headless Alacritty grid harness, and a long-lived portable-pty session manager keyed by surface ID.
Updated: 2026-07-06 (Phase 3 terminal I/O slice): added native `surface.send_text`, `surface.send_key`, `surface.read_text`, resize/kill plumbing, CLI `send`/`send-key`/`read-screen`/`trigger-flash`, and a path-filtered Windows Rust CI workflow.
Updated: 2026-07-06 (Phase 3 zoom/projection slice): added backend-owned pane zoom state, `pane.zoom`/`zoom-pane` parity, and a `pandamux-ui` shell projection that maps a workspace split tree into visible panes for the future Iced shell.
Updated: 2026-07-06 (Phase 3 Iced shell slice): added a feature-gated `pandamux-ui::iced_shell` module with a canvas terminal viewport, pane/tab controls, split rendering, and CI coverage via `cargo test -p pandamux-ui --features iced-runtime --lib`.
Updated: 2026-07-06 (Phase 3 app runtime slice): added `pandamux-app --features iced-runtime -- --iced-shell` runtime wiring, stable `ShellViewModel` ownership, `ShellMessage` to core-intent routing, and CI coverage via `cargo test -p pandamux-app --features iced-runtime --bin pandamux-app`.
Updated: 2026-07-06 (Phase 3 live Iced PTY slice): replaced the Iced runtime launcher path's seeded placeholder terminal snapshots with live `PtySessionManager` snapshots, while keeping deterministic placeholder snapshots for non-live unit tests.
...
Updated: 2026-07-08 (Phase 4 backend + terminal-adjacent UI complete): landed the hand-built terminal engine (`pandamux-term::{search, links}` + `TerminalGrid::{serialize, serialize_lines, search, links, cursor}`), ported PTY lifecycle semantics (`pandamux-term::shell`: shell resolution, chunked writes, DA1/CPR interception, resize no-op drop, Windows tree-kill, POSIX/WSL cwd), the notifications model (`pandamux-core::notification`), session persistence (`pandamux-app::persistence`: atomic autosave + named sessions + version-change), and the terminal-adjacent UI (find bar with live match highlight, link underlines, copy-mode indicator, 320px notifications slide-over with bell unread dot, session-activity status dot). Validated: `cargo fmt --check`, `cargo clippy` (no new warnings), and the full suite (`pandamux-term` 26, `pandamux-core` 5+14+5, `pandamux-app` 28, `pandamux-ui` 17) plus `--iced-shell-smoke`. Remaining Phase 4 items are tracked as deferrals under the Phase 4 section. See the Phase 4 gotchas in Section 10.
Updated: 2026-07-08 (Phase 3 design build-out complete): all seven Phase 3 design deliverables are landed. Added `pandamux-ui::theme` (all Section 12 tokens), `pandamux-ui::chrome` (40px titlebar, 52px icon rail, 26px toggleable status bar), a `columns` projection on `ShellProjection`, a restyled pane frame + fixed-dark terminal viewport with a blinking block cursor, and `app_view` composing the full chrome. The runtime owns chrome view state, drives window controls through Iced window tasks, and maps Ctrl-shortcuts. Validated: `cargo fmt --check`, `cargo clippy` (no new warnings), 14 `pandamux-ui` + 20 `pandamux-app` tests, and the compiled `--iced-shell-smoke` path (`PANDAMUX_ICED_SHELL_SMOKE_OK`). The crate-isolation invariant holds (zero Iced in core/term). Phase 3 is complete; next is Phase 4 (terminal-adjacent parity). See the Phase 3 design gotchas in Section 10.
Updated: 2026-07-08 (Phase 5 COMPLETE): the nine remaining Phase 5 to-dos shipped across versions 0.24.0-0.32.0 as sequential, individually fmt/clippy-clean, unit-tested waves: (1) orchestrator env plumbing + `agentId` + a hermetic wire-compat regression test; (2) markdown/diff surfaces + `SurfaceContents` store + the `DispatchCtx` refactor; (3) drag-and-drop pane splitting (`move_surface` + drop zones); (4) terminal themes + WT/Ghostty import + i18n; (5) the full keyboard set + palette arrow-nav; (6) per-session cwd tracking (OSC 9;9/7 + `report_pwd`); (7) multi-window parity + surface color scheme + AppUserModelID; (8) canvas line icons + bg gradient; (9) the `claude_context` startup integration (CLAUDE.md injection + orchestrator plugin install). Tracked deferrals: the JetBrains Mono TTF (asset drop-in), radial glow + glass blur (no Iced primitive), the `settings.json` hook wiring + live activity observer (Phase 7 boundary), copy-mode yank/link-open (pairs with Phase 6 OSC 52), and animation polish. Next is Phase 6 (SSH/OSC-52/SFTP new features).
Updated: 2026-07-08 (Phase 6 COMPLETE, shipped 0.33.0): F1 (OSC 52 copy/paste + bracketed paste, forwarded to the OS clipboard via arboard), F2 (SSH remote surfaces with a `RemoteSessionManager` promoted from the Phase 2 russh spike: tmux `new-session -A` durability, backoff reconnect + reset-on-reattach, `window-change` resize, agent/key/password auth; host profiles + `~/.ssh/config` import; SSH context chip), and F3 (SFTP image paste to `/tmp/pandamux-paste-<uuid>` + path injection) all landed and are fmt/clippy-clean and hermetically unit-tested, with an opt-in `--ssh-smoke` for live validation. See the Phase 6 section for tracked follow-ups (known-hosts pinning, ProxyJump dialing, Pageant bridge, interactive vi-nav copy mode + link-click). Also refined Phase 7: the release is one GitHub Actions workflow that builds, Azure-signs, and publishes a single signed `.exe` installer (Velopack); the Electron files are a never-shipped fork pull to be deleted, so the Electron session-import migration is dropped.
...
### Phase 3: Foundation
- [x] Cargo workspace scaffold; enforce the 6.1 crate-isolation boundary in CI from day one.
- [x] pandamux-core split tree (port split-utils.ts semantics + its tests first, they are the best-specified logic in the repo).
- [x] Implement the backend-owned-state model per Section 6.2 (intent-in, delta-out; single writer; Iced read-projection). This is the load-bearing architectural work; do it before building UI breadth on top.
- [x] Stand up the testing harnesses from Section 6.3 (core unit tests, headless grid harness, pipe wire-compat suite).
- [x] Named-pipe server in tokio, wire-compatible V1 + foundational non-browser V2 for system/workspace/layout/pane/surface commands; port pipe-server tests.
- [x] pandamux-cli against it (`ping`, `identify`, `tree`, workspace/pane/surface commands, `zoom-pane`, and `layout grid` because the orchestrator depends on it).
- [x] Native terminal I/O foundation: write text, send keys, resize/kill PTY sessions, read screen text from the headless grid, and smoke through the compiled pipe server and CLI.
- [x] Path-filtered Windows Rust CI for formatting, boundary checks, tests, and native binary builds.
- [x] Pane zoom parity: core state, V2 `pane.zoom`, CLI `zoom-pane`, live pipe smoke, and zoom-aware shell projection.
- [x] Feature-gated Iced shell view: projection-to-Iced split rendering, tabs, focus/zoom/surface messages, canvas terminal viewport, and Iced feature compile/test coverage.
- [x] Feature-gated app runtime shell wiring: app-owned `ShellViewModel`, `ShellMessage` routing into core intents, optional `--iced-shell` entrypoint, and app feature compile/test coverage.
- [x] Live Iced runtime terminal snapshots: `--iced-shell` runtime owns PTY sessions by surface ID and refreshes `TerminalSnapshot` from Alacritty grid text.
...
- [x] Central theme module in `pandamux-ui` encoding every Section 12 token: dark + light chrome palettes, the configurable accent set, typography scale (Segoe UI + JetBrains Mono), radii, shadows, spacing. Everything downstream consumes this; no hardcoded colors in widgets. Note: the mono face is referenced as a named font (`JetBrains Mono`, single reference point `theme::MONO_FONT`) that falls back to the system monospace; bundling the actual TTF bytes is a one-line drop-in deferred to a later pass.
- [x] Frameless window + 40px custom titlebar: logo mark, session-switcher pill (opens palette), bell with unread dot, settings icon, min/max/close hit areas, whole-bar drag region (window drag/minimize/toggle-maximize/close wired through Iced window tasks).
- [x] 52px icon rail (Sessions / palette / new / notifications / settings-pinned-bottom) with active + hover states.
- [x] 26px toggleable status bar (shell indicator, git branch + ahead-count, ports, session/pane counts, encoding, version). Git/ports segments render when the (Phase 5) pollers supply data; shell/counts/encoding/version are live now.
- [x] Pane workspace frame styled to spec: column layout with 8px gaps / 10px padding, 36px per-pane tab bar (shell glyphs + close + split-right/split-down + zoom buttons), focus ring, pane shadow, radii. SSH context chip lands with remote surfaces (Phase 6).
...
### Phase 4: Terminal-adjacent parity (backend + terminal-adjacent UI COMPLETE 2026-07-08)
The terminal engine and everything that lives close to the grid.
- [x] Hand-built terminal search, serialize (scrollback + visible), link detection, exercised through the headless grid harness (`pandamux-term::{search, links}`, `TerminalGrid::{serialize, serialize_lines, search, links, cursor, history_size}`).
- [x] Find-in-terminal (bar + match count + next/prev + case toggle + current-match highlight over the focused terminal), links UI (accent underline of detected URL spans). Copy mode: enter/exit toggle + visible indicator (full vi-navigation selection + OS-clipboard yank tracked as a deferred item, pairs with F1 OSC52/clipboard in Phase 6).
- [x] PTY lifecycle semantics ported from pty-manager.ts: shell resolution (pwsh>powershell>cmd), chunked writes (>1KB into 1KB chunks), DA1 + CPR query interception, resize no-op drop, Windows process-tree kill (`taskkill /T /F`), and POSIX/WSL cwd translation primitive. Pure logic unit-tested (`pandamux-term::shell`).
- [x] Session persistence: atomic auto-save session.json (30s cadence wired into the runtime), named sessions (save/load/list/delete, sanitized names), version-change handling (clear auto-session only, preserve named + last-session pointer). Unit-tested against a temp dir (`pandamux-app::persistence`).
- [x] Notifications: backend store (max 200, evict-read-first, add/mark-read/mark-all/clear/jump/unread-count) in `pandamux-core::notification`, wired into the runtime with a bell unread dot and the 320px panel.
- Exit criteria (mostly met): a keyboard-driven multi-pane terminal is usable with search, persistence, and notifications. Copy-mode selection/yank depth and the CLI/pipe -> live-UI bridge are the remaining pieces, tracked below.

Design deliverables (per Section 12; the terminal-adjacent surfaces):
- [x] Find bar and links UI styled to the chrome token system; copy-mode indicator styled to spec.
- [x] Notifications panel: 320px right-side slide-over, "Clear all" action, source-dotted cards (build/agent/deploy/port/generic), plus the titlebar bell unread dot. (Slide-in animation and the notification "ring" flash are deferred with the animation-polish items.)
- [x] Session status plumbing: running (accent) / busy-agent (gold) / idle status-bar dot fed by the PTY layer; busy-agent detection is wired to the (Phase 5) agent observer.

Deferred from Phase 4 (tracked; each has a working baseline in place now):
- [ ] Copy mode: full vi-style selection navigation (hjkl/arrows, visual selection region) and OS-clipboard yank. Pairs naturally with F1 (OSC 52 + clipboard via `arboard`) in Phase 6; the mode toggle + indicator + scrollback-freeze are in place now.
- [ ] Link click-to-open: canvas hit-testing of the underlined span + OS opener. Detection and underline rendering are done; opening is deferred.
- [ ] Find over full scrollback + scroll-to-match: the engine's `TerminalGrid::search` already covers scrollback; the find UI currently searches the visible screen so match spans map directly to viewport rows. Extend to scrollback with a scroll-to-match once the viewport gains scroll offset.
- [ ] CLI/pipe -> live-UI bridge for `notify` and `read-screen`: today the pipe server and the Iced runtime are separate entrypoints with separate state. Unifying them (the single-writer intent/delta channel from Section 6.2) so `pandamux notify` reaches the running UI and `read-screen` returns the live serialize is part of the Phase 5 pipe/agent/orchestrator integration.
- [ ] Full PTY spawn-args assembly: integration-script injection, WSL `--cd`, and `PANDAMUX_*` env plumbing on session spawn. The lifecycle primitives (resolution/chunking/DA1/tree-kill/posix-cwd) are done; the spawn-args assembly rides the Phase 5 shell-integration + Claude-context work.
- [ ] Notifications slide-in (160ms) + notification-ring flash animation; OS toast bridge (`notify-rust`/Windows toast).

### Phase 5: Peripheral UI and integrations parity (IN PROGRESS)
The rest of the Section 4 checklist.

Delivered so far (2026-07-08, versions 0.18.0-0.23.0):
- [x] **Pipe/UI unification (load-bearing, Section 6.2).** The named-pipe server and the live Iced runtime share one synchronous dispatcher (`pandamux-app::backend::handle_line`); the live shell embeds the pipe server as an Iced subscription (`ShellMessage::PipeRequest` + oneshot registry), so a CLI/agent/orchestrator `split`/`notify`/`read-screen`/workspace-pane-surface command applies to the same canonical state the UI owns and repaints live. Single writer; both clients hit the same code path.
- [x] Notifications over the pipe: `notification.raise`/`list`/`clear` methods + `notify`/`list-notifications`/`clear-notifications` CLI commands reach the running UI's bell and panel.
- [x] **264px session panel.** Sessions projected across workspaces (each terminal surface is a shell context), Project/Type/Host live-regroup switcher, pinned-first active group, shell-badge rows, "+ New session" footer; selecting focuses the surface + activates its workspace (no layout swap, per 12.1 #2). Sessions rail toggles the panel.
- [x] **Command palette (Ctrl+K)** with live substring filter over commands + pane actions + session switches; **quick-launch popover** (PowerShell 7/5.1, CMD, WSL profiles) opened by the rail "+", panel footer, and Ctrl+T; **settings modal** (640x440, left nav General/Terminal/Keyboard/Notifications/Quick launch) with live UI-theme/accent/status-bar controls and the keyboard list. One overlay at a time over a dismiss-on-backdrop scrim; Esc closes; Ctrl+, opens settings.
- [x] **Git + port pollers** feeding the status bar (git branch/ahead in the process cwd; localhost dev-port scan), driven off the Iced timer via async tokio I/O.
- [x] Browser/CDP: native pipe + CLI reject `browser.*`/`browser` with a clear "use Claude Code's browser tooling" message; capabilities already reports `browser: false`. (Rewriting the shared Electron-facing docs/injected instructions is deferred to the Phase 7 Electron-deprecation boundary, since the Electron build still ships the browser pane.)
- [x] **Agent manager** (`agent spawn`/`spawn-batch`/`status`/`list`/`kill`, round-robin/stack/split distribution) in `pandamux-core::agent` + the shared dispatcher; an agent is a terminal surface running a command, threaded through the single-writer path so orchestrator-spawned agents appear live. Busy-agent status dot wired.
- [x] **Sidebar status/progress/log** (`set-status`/`set-progress`/`log`/`sidebar-state` CLI + `sidebar.*` pipe methods) in `pandamux-core::sidebar`; progress shown in the status bar. Together with the agent manager this is the orchestrator's coordination surface. (Regression-testing the orchestrator's actual scripts against the Rust pipe server still remains.)

Remaining (tracked to-dos): **all delivered 2026-07-08 across versions 0.24.0-0.32.0 (nine sequential waves, each fmt/clippy-clean and unit-tested).**
- [x] pandamux-orchestrator plugin verification (0.24.0): `PANDAMUX_*` env plumbing on spawn (incl. `PANDAMUX_AGENT_ID` for the hooks), `agentId` alias on `agent spawn`/`list`, and a hermetic wire-compat regression test that replays the exact method sequence the scripts invoke and asserts every response field `json-tool.js` parses.
- [x] Drag-and-drop pane splitting (0.26.0, Section 12.3): core `move_surface` + `DropZone` (center append / directional split, prune-empty, no-op on a pane's only tab), five drop-zone overlays, dimmed source tab, accent zone highlight, a movement gate standing in for the 6px threshold. Ghost chip + 100ms zone-fade animation deferred with the other animation polish.
- [x] Claude Code integration (0.32.0): `claude_context` module (context injection into `~/.claude/CLAUDE.md`, orchestrator plugin install into the plugin cache + `installed_plugins.json` + `settings.json` enable), idempotent, run best-effort on GUI launch. The `settings.json` hook wiring + live activity observer stay deferred to the Phase 7 boundary (busy-agent dot is the interim signal).
- [x] Markdown + diff surfaces (0.25.0): `SurfaceContents` store threaded through the dispatcher, `markdown.set_content`/`markdown.load_file`/`diff.set_content`/`diff.refresh` + CLI, line-based markdown + colored-diff renderers. Folded the dispatcher's `&mut` list into a `DispatchCtx` struct.
- [x] Themes: theme JSON loading + WT/Ghostty import; i18n (0.27.0): bundled `.theme` loading, WT/Ghostty importers, `theme.*`/`config.*` + CLI, a `TermScheme` applied to the viewport, and an `i18n` locale catalog.
- [x] Full designed keyboard set + palette arrow-key navigation (0.28.0): Ctrl+D/Shift+D/W/Enter/Shift+P focused-pane actions and Up/Down/Enter palette nav.
- [x] Per-session cwd tracking (0.29.0): OSC 9;9 / OSC 7 stream parsing + V1 `report_pwd`, scoping the git poller to the focused session.
- [x] Multi-window parity + surface color scheme + config show/reload + AppUserModelId (0.27.0/0.30.0): `window.list`/`focus`, `surface.set_color_scheme`/`clear`, `config.show`/`reload`/`path`, and the `com.pandamux.app` AppUserModelID.
- [x] Visual polish (0.31.0): a canvas line-icon set replacing the unicode placeholders + the design's vertical bg gradient. JetBrains Mono TTF (asset), radial glows, and glass blur remain deferred (see below).
- Exit criteria met: every Section 4 non-browser box is delivered (SSH/OSC-52 clipboard and image-paste are Phase 6). Phase 5 parity is declared, minus the tracked visual-asset deferrals below.
...
- Session panel (264px, compact 216px): the 3-segment Project / Type / Host grouping switcher (live regroup), pinned-first groups, session rows (shell badge / name+meta / status dot), "+ New session" dashed footer. Wires the workspace+session model (Section 12.2): sessions are shell contexts indexed across workspaces. Select-session behavior is settled (owner-confirmed 2026-07-08, Section 12.1 #2): selecting focuses/activates the session's pane (opening it in the current workspace if unplaced), never swaps the layout wholesale.
...
- [x] DONE 0.31.0. Replaced the unicode glyph placeholders (titlebar, icon rail, tab bar, status bar) with the design's 1.2-1.4px-stroke line icons drawn as Iced canvas paths (`pandamux-ui::icons`). No icon font.
- [x] DONE 0.31.0 (gradient); radial glows still deferred. The `bg-base` vertical gradient is live (`Palette::bg_gradient` -> `iced::gradient::Linear`). The teal (top-right) and gold (bottom-left) radial ambiences have no Iced primitive and remain deferred (approximate with layered translucent shapes or a pre-rendered backing image).
...
### Phase 6: New features (DELIVERED 0.33.0)
Status: **DONE 2026-07-08** (hermetically unit-tested; the live SSH/SFTP paths are exercised end-to-end via `pandamux-app --ssh-smoke` against a real host, since they cannot run in headless CI).
- [x] F1 OSC 52 copy/paste + bracketed paste. `pandamux-term::clipboard` (capturing grid `EventListener`, `Osc52::OnlyCopy` policy, size-capped store, deny-load-by-default with per-host opt-in, `wrap_paste`), forwarded to the OS clipboard via `pandamux-app::clipboard_os` (arboard) on every UI refresh + after each headless pipe command. Pipe: `clipboard.copy`/`get`/`policy`, `surface.paste`. CLI: `clipboard copy|get|policy`, `paste`.
- [x] F2 SSH remote surfaces + tmux durability + reconnect. `pandamux-term::ssh::RemoteSessionManager` (promoted from the Phase 2 russh spike; sync API mirroring `PtySessionManager`), `tmux new-session -A` durability with login-shell fallback, backoff reconnect + reset-on-reattach reconciliation (Section 5), `window-change` resize, agent/key/password auth. Backend routes terminal I/O to the remote session for remote surfaces; the UI shows an SSH context chip. Pipe: `ssh.connect`/`disconnect`/`list`/`profiles`/`save_profile`/`remove_profile`/`import_config`. CLI: `ssh connect|disconnect|list|profiles|save-profile|import`. Host profiles + `~/.ssh/config` import in `pandamux-core::ssh`.
- [x] F3 SFTP image paste/drop. `RemoteSessionManager::upload_image` (russh-sftp) to `/tmp/pandamux-paste-<uuid>` + remote-path injection; local surfaces inject the local path. Pipe: `surface.paste_image`. CLI: `paste-image`.
- Tracked follow-ups (not blockers): known-hosts pinning (the connection currently accepts the server key, matching the spike); ProxyJump dialing (parsed/stored, not yet dialed); the optional Pageant agent bridge (not required, since agent/key/password auth already pass); full interactive vi-navigation copy mode + canvas link-click hit-testing (the GUI-interaction halves of the paired Phase 4 deferrals; the yank extraction primitive `TerminalGrid::region_text` + `clipboard.copy` are done).

### Phase 7: Ship
Status: **CODE/INFRA COMPLETE 2026-07-10 (0.35.0), pending the first real signed release.** Landed: single-source versioning (root `[workspace.package] version = 0.35.0`, crates inherit); winresource `build.rs` embedding the icon + version into `pandamux.exe` (release build confirmed showing the metadata); the GUI binary renamed to `pandamux.exe` via `[[bin]]`; cargo-packager NSIS config (`[package.metadata.packager]`), validated locally end-to-end (`cargo packager` produced `pandamux_0.35.0_x64-setup.exe`); the Rust `release.yml` (build -> Azure-sign exes -> cargo-packager -> sign installer -> single-asset Release); winget retargeted at the NSIS installer; the full Electron prototype deleted (pure Rust workspace); and the docs (CLAUDE.md/AGENTS.md/README/injected instructions/commit rules) moved to the native app. Remaining, each externally gated: (1) the owner tags `v0.35.0` to run the real signed release (CI does the signing); (2) the public `site/` rewrite lands with that release (deferred so as not to misrepresent the current download); (3) the winget-pkgs package needs a one-time manual re-bootstrap PR because the installer type changed from zip to nsis; (4) the `docs/` wiki (Electron-era) gets a `doc-sync` regeneration pass. The updater download-and-run side effect is wired against the real `Setup.exe` asset shape.

The whole release runs in GitHub Actions on a version tag; a person never runs the build or the signing locally, and the GitHub Release carries exactly one asset: the signed `.exe` installer.
- **CI build + sign + publish (one GitHub Actions workflow, `windows-latest`, triggered on a `v*` tag).** Steps: `cargo build --release` producing `pandamux.exe` (icon + version metadata embedded via `winresource` in build.rs) -> sign `pandamux.exe` with **Azure Trusted Signing** (the `AZURE_*` service-principal secrets from Phase 2, sourced via Doppler; unsigned fallback only if a signing run fails) -> wrap the signed exe into a single `Setup.exe` installer -> sign the installer -> create the GitHub Release with the installer as its only asset.
- **Installer generator: cargo-packager (decided 2026-07-10).** Because updates go through our own in-app checker (below), we do NOT need Velopack's updater feed (`releases.win.json` + `.nupkg`), so the release stays exe-only. cargo-packager (NSIS format) was chosen over Inno/Velopack: config lives in `[package.metadata.packager]` in `crates/pandamux-app/Cargo.toml`, it downloads its own NSIS (no runner install), and crucially it does NOT rebuild the binary (no `before-packaging-command`), so it bundles the already-Azure-signed `pandamux.exe` unchanged and the signature stays valid. Kebab-case keys, but note `installer-mode` (not `install-mode`). The old Electron `latest.yml`/feed requirement (issue #68) does not carry over.
- **In-app update checker (`pandamux-app::updater`, built in 0.34.0 ahead of packaging).** On launch and on an interval, the app calls the GitHub Releases API (`/repos/BoardPandas/Pandamux/releases/latest`), normalizes + semver-compares the tag to `CARGO_PKG_VERSION`, and (past a quarantine window, porting `updater.ts`'s quarantine semantics) raises a toast through the existing notification model offering to update. On accept it downloads the release's `Setup.exe` asset, launches it, and exits so the installer replaces files; Windows verifies the installer's Authenticode signature on run. The hermetic half (API parse, semver compare, quarantine gate) is unit-tested now; the download-and-run side effect is wired when packaging lands and there is a real release to point at. Drafts/prereleases are never offered.
- Docs (not an Electron migration, just accuracy): update `resources/claude-instructions/claude-instructions.md` (drop the `pandamux browser` guidance, Section 4.1), README CLI examples, and `pandamux.boardpandas.ai`.
- Delete the never-shipped Electron files from this repo (`src/`, `package.json`, `electron-builder.json`, the ASAR release doc, etc.). The Electron build here was a fork pull that was never used, so there is no user base and no sessions to migrate; this is a plain file deletion, not a deprecation/transition.
...
- (Phase 3) 2026-07-06 foundation start: the production root `Cargo.toml` must explicitly `exclude = ["spikes/phase2-native-terminal"]`. Without that, Cargo treats the disposable spike as part of the new workspace and its own manifest reports that it belongs to the parent workspace. Keep spike crates outside production unless they are intentionally promoted.
- (Phase 3) 2026-07-06 validation gotcha: run Cargo validation serially in this repo. Parallel `cargo test`, `cargo tree`, and spike checks contend on the package cache and build directory, producing noisy file-lock waits and misleading transitive build failures. Use one Cargo command at a time for reliable evidence.
- (Phase 3) 2026-07-06 PTY session gotcha: long-lived portable-pty sessions must retain the `MasterPty` handle in the session struct. Keeping only cloned reader and writer handles can produce an apparently live session with no output bytes. The passing session manager stores the master, child, writer, read channel, CPR state, and grid together.
- (Phase 3) 2026-07-06 Windows validation gotcha: Windows Application Control can intermittently block freshly rebuilt Cargo test executables with `os error 4551`. In this run, targeted package invocations and package or target cleans cleared the block, and the same tests passed. Treat this as host-policy noise only after the affected package test passes on a targeted rerun.
- (Phase 3) 2026-07-06 terminal I/O result: the native `read-screen` path now returns real Alacritty grid text instead of the Electron-era serializer stub. Live smoke passed by sending `Write-Output PANDAMUX_NATIVE_IO_OK` into a `pwsh` PTY through `surface.send_text` + `surface.send_key`, then polling `surface.read_text` until the marker appeared.
- (Phase 3) 2026-07-06 zoom/projection result: `zoom-pane` now toggles backend-owned `WorkspaceState.zoomed_pane_id`, clears on topology rewrites, and is reflected by `pandamux-ui::project_workspace_shell` as a single visible pane while preserving the full split tree in core state.
- (Phase 3) 2026-07-06 Iced shell result: `pandamux-ui` now has an `iced-runtime` feature that builds `ShellProjection` into Iced elements without making UI state authoritative. CI now runs `cargo test -p pandamux-ui --features iced-runtime --lib` so the pinned Iced API is checked explicitly. Runtime app wiring and visual QA remain the next Phase 3 UI work.
- (Phase 3) 2026-07-06 app runtime result: `pandamux-app` now has an `iced-runtime` feature with an optional `--iced-shell` launcher. The runtime stores a stable `ShellViewModel` instead of returning Iced elements that borrow a temporary model, and routes pane focus, pane zoom, and surface focus messages back through core intents. CI now also runs `cargo test -p pandamux-app --features iced-runtime --bin pandamux-app`.
- (Phase 3) 2026-07-06 live Iced PTY result: `NativeShellRuntime::new(true)` now owns a `PtySessionManager`, syncs terminal surfaces into PTY sessions, and builds `TerminalSnapshot` from `screen_text_lines`. The ignored live test `live_runtime_snapshots_capture_pty_output` passed by writing `PANDAMUX_ICED_RUNTIME_PTY_OK` into the runtime PTY and observing the marker through the Iced view model.
- (Phase 3) 2026-07-06 Iced refresh and smoke result: `pandamux-app --features iced-runtime -- --iced-shell` now subscribes to `iced::time::every` and maps ticks into snapshot refreshes without mutating core state. The noninteractive `--iced-shell-smoke` path builds the app model and Iced view once, prints `PANDAMUX_ICED_SHELL_SMOKE_OK`, and is covered in Rust CI.
- (Phase 3) 2026-07-06 Iced shell controls result: `ShellMessage` now covers split horizontal, split vertical, new terminal tab, close tab, and close pane. `NativeShellRuntime` maps each one into `PaneIntent` or `SurfaceIntent`, so the Iced layer remains a read projection plus intent source instead of a second state owner.
- (Phase 3) 2026-07-06 interactive Iced smoke result: the original `--iced-shell` launch panicked because `#[tokio::main]` created a runtime before Iced started its tokio-backed event loop. The app entrypoint now launches Iced before creating Tokio, and the pipe-server path creates Tokio explicitly only when needed. Manual interactive smoke passed after the fix.
- (Phase 3) 2026-07-08 design build-out: Iced 0.14 API gotchas hit while replicating the design. `Space` has no `with_width`/`with_height`/`new(w, h)`; use `Space::new().width(..).height(..)`. Style structs (`container::Style`, `button::Style`) implement `Default`, so build them with `..Default::default()`; `Border`/`Shadow` derive `Default`. `container.style()` takes `Fn(&Theme) -> Style`; `button.style()` takes `Fn(&Theme, button::Status) -> Style`. `Padding` has `From<f32>`/`From<u16>` but not `From<i32>`, so `.padding(0.0)` not `.padding(0)`. `Alignment::{Center,End}` convert into both `alignment::Horizontal` and `alignment::Vertical`, so they work for container `align_x`/`align_y` and row/column alignment. Window controls need a window `Id`: `iced::window::latest()` returns `Task<Option<Id>>` and `Task::<Option<T>>::and_then(fn(Id) -> Task<_>)` chains cleanly to `window::drag`/`minimize`/`toggle_maximize`/`close`; the app `update` fn must therefore return `Task<Message>` (the functional `application(..)` accepts it). Keyboard shortcuts: `iced::keyboard` only re-exports `listen() -> Subscription<Event>` (no `on_key_press`); map events to messages and use a benign `Noop` message for non-shortcut keys since `Subscription::map` cannot filter.
- (Phase 3) 2026-07-08 palette design: keeping `Palette` a `Copy` struct (all `Color`/enum fields) lets it be captured by value into every Iced style closure without lifetime friction, which is what made the "no hardcoded colors in widgets" rule practical. The overlay tint (`Palette::ov(alpha)`) flips white/black by theme so one call site serves both chrome themes. Functional-record-update (`Color { a, ..color }`) is not allowed in a `const fn`, so `with_alpha` is a plain fn.
- (Phase 3) 2026-07-08 Windows Application Control recurrence: the `os error 4551` build-script block (first seen 2026-07-06) reappeared on `proc-macro2`'s freshly built `build-script-build`. `cargo clean -p proc-macro2` followed by a rerun cleared it and the build/tests passed. Treat as host-policy noise: targeted-package clean + rerun, not a code problem. (Recurred again in Phase 4 on freshly built test executables; same fix: rerun, or `cargo clean -p <pkg>` then rerun.)
- (Phase 4) 2026-07-08 terminal engine: search/serialize/links are built on character offsets (grid columns), not byte offsets, so wide/CJK glyphs do not shift reported columns; match/link spans line up with the rendered grid. Link detection is hand-rolled (a small scheme scanner for http/https/file/ftp + bare `www.`, trimming trailing sentence punctuation) rather than pulling in `linkify` -- dependency-free, offline-safe, and fully unit-testable; swap to `linkify` later only if richer TLD/edge coverage is needed. Serialize walks scrollback via `grid[Line(-history..screen)]` where `history = total_lines - screen_lines`.
- (Phase 4) 2026-07-08 PTY lifecycle: keep the pure, effect-free logic (shell resolution ordering, write chunking, DA1/CPR query detection, POSIX-path/win32-cwd translation) in `pandamux-term::shell` so it is unit-testable without spawning a shell; the effectful pieces (spawn, `taskkill /T /F` tree-kill via `Child::process_id()`) stay in `session.rs`. DA1 query detection must reject the reply/DA2/DA3 forms (a `?`/`>`/`=` private marker right after `ESC [`), else you answer your own reply in a loop. Resize must drop same-size resizes (they make the shell redraw the prompt -- a doubled-prompt cause); track the last size on the session.
- (Phase 4) 2026-07-08 Iced 0.14 UI: `text_input::Style` has NO `Default` -- construct all six fields (`background`, `border`, `icon`, `placeholder`, `value`, `selection`); its `.style()` closure is `Fn(&Theme, text_input::Status) -> Style`. `Padding` implements `From<[f32; 2]>` and `From<f32>` but NOT `From<[f32; 4]>`; for four-sided padding use the struct literal `Padding { top, right, bottom, left }`. Right-side slide-over overlays compose with `iced::widget::stack![base, overlay]` where the overlay is a full-size container that aligns its 320px panel to `End`/`Start`; a non-interactive container overlay lets clicks fall through to the base while the panel's own buttons capture theirs.
- (Phase 4) 2026-07-08 persistence: `SessionStore` takes its base dir by value so tests run against a temp dir and the live app resolves `%APPDATA%/pandamux`; only the live runtime (`live_ptys`) touches disk, keeping tests/smoke hermetic. Named-session `AppState` roundtrip tests must capture the saved instance and compare against it -- `split_state()`-style helpers mint fresh uuids each call, so re-generating for the assertion fails equality. The named-session half of the API (save/load/list/delete) is complete and tested but not yet wired to a UI trigger (Phase 5 command palette / session panel), so the `persistence` module carries `#[allow(dead_code)]` deliberately.
- (Phase 5) 2026-07-08 pipe/UI unification: the single-writer bridge (Section 6.2) is an embedded pipe server running as an Iced subscription. Key mechanics: (1) `ShellMessage` must stay `Clone + PartialEq + Eq` (tests assert on it and Iced needs Clone), so the message carries only `PipeRequest { id: u64, payload: String }` (no oneshot sender, which is none of those); the `oneshot::Sender<String>` lives in a side `Arc<Mutex<HashMap<u64, _>>>` registry keyed by id. (2) `Subscription::run_with(data, builder)` takes `builder: fn(&D)` (a plain fn pointer, cannot capture), and `D: Hash`; pass the shared `Arc` handles inside a `D` struct whose `Hash` impl hashes only a stable id string so the subscription is spawned once and survives view rebuilds. (3) The builder must return a concrete boxed stream (`Pin<Box<dyn Stream + Send>>`), NOT `impl Stream` -- the opaque return is treated as borrowing `&D` and fails the `fn(&D) -> S` coercion. (4) The shared dispatcher is a free sync fn `backend::handle_line(&mut AppState, &mut PtySessionManager, &mut Notifications, &mut u64, now_ms, spawn_ptys)`; the standalone server calls it under a tokio mutex, the runtime calls it directly in `update` -- one code path, so CLI- and UI-driven mutations are identical at the state layer.
- (Phase 5) 2026-07-08 pollers: run git/port polling off `iced::time::every` -> a `PollRequested` message handled in `update` as `Task::perform(poll_all(), PollUpdate)`, keeping the poll in the async pool and off the sync `update`. Use async tokio only (`tokio::process::Command`, `tokio::net::TcpStream` + `tokio::time::timeout`), never blocking `std::fs`/`std::process` on the runtime (LL-G Rust HIGH gotcha). `PollUpdate` results are written into chrome and persist across refreshes because `rebuild_chrome` does not touch the git/ports fields. Needed `tokio` features `process` + `time` on `pandamux-app`.
- (Phase 5) 2026-07-08 overlays: one centered overlay at a time is tracked as `chrome::Overlay` (None/CommandPalette/QuickLaunch/Settings); `app_view` layers the active overlay over the base via `stack!`. The shared scrim wrapper (`command_palette::modal`) is `pub(crate)` and reused by settings; the card wraps its content in a `mouse_area(..).on_press(Noop)` so inside-clicks do not fall through to the backdrop's `OverlayDismissed`. Palette rows carry their own `ShellMessage` action (clicking dispatches it directly); the runtime rebuilds the full item list each refresh and `filter_items` narrows by the live query.
- (Phase 5) 2026-07-08 session model: sessions are a pure projection over `AppState` (every terminal surface across all workspaces), not new canonical state, which keeps the split tree canonical (decision 12.1 #1/#2). `project_sessions(&AppState, grouping, open, active_surface)` lives in `pandamux-ui` (it depends on `pandamux-core`), so the crate-isolation invariant holds (no Iced in core/term). Selecting a session applies `WorkspaceIntent::Select` then `SurfaceIntent::Focus` -- focus/activate, never a layout swap.
...
2. **Workspace and session are both kept.** Workspace = the on-screen split layout of panes. Session = a named shell context (project . host . shell . status . branch/activity meta) that the 264px panel indexes and groups by Project/Type/Host. **Resolved 2026-07-08 (owner-confirmed): selecting a session focuses/activates its existing shell context; it does NOT swap the on-screen layout wholesale.** Sessions are shell contexts surfaced across workspaces; selecting one focuses its pane, opening it in the current workspace if unplaced. This keeps the binary split tree canonical (no session-owned layout state), and the "load a saved layout" behavior stays the job of workspace select / session persistence, not session select.
