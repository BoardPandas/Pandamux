...
- **Installer generator is a free pick.** Because updates go through our own in-app checker (below), we do NOT need Velopack's updater feed (`releases.win.json` + `.nupkg`), so the release stays exe-only. The installer can be produced by whatever is simplest (`cargo-packager`, Inno Setup, or Velopack used purely as a packager); decide in a small packaging spike. The old Electron `latest.yml`/feed requirement (issue #68) does not carry over.
