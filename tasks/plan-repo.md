...
Remaining (tracked to-dos):
- [ ] pandamux-orchestrator plugin verification: regression-test its scripts against the Rust pipe server end-to-end (the methods it calls -- `layout grid`, `agent *`, `sidebar *`, `notify` -- are all implemented now).
- [ ] Drag-and-drop pane splitting (Section 12.3): 6px threshold, dimmed source tab, accent ghost chip, position-based drop zones + overlay, split/move drop semantics kept 2-level. (Tab-bar split/zoom/close buttons already exist.)
- [ ] Claude Code integration (startup/filesystem): context injection into `~/.claude/CLAUDE.md`, hooks config, orchestrator plugin install, activity observer. Largely overlaps the frozen Electron `claude-context.ts`; best done near the Phase 7 ship boundary. (The busy-agent dot is already fed by the agent registry as an interim signal.)
- [ ] Markdown surface (evaluate Iced's `markdown` module) + diff surface, with the `markdown.set_content`/`markdown.load_file`/`diff.refresh` pipe methods and a per-surface content store (non-terminal surfaces currently render a placeholder). Note: this needs a content store threaded through the dispatcher + view; a good moment to fold the dispatcher's growing `&mut` param list into a single `DispatchCtx` struct.
- [ ] Themes: theme JSON loading + WT/Ghostty config import; i18n.
- [ ] The full designed keyboard set beyond the currently wired shortcuts, and palette arrow-key navigation (click + Enter work now).
- [ ] Per-session cwd tracking (shell-integration OSC reporting) so the git poller scopes to the focused session rather than the process cwd.
- [ ] Multi-window parity (`list-windows`/`focus-window`), surface color scheme (`set-color-scheme`), config show/reload, AppUserModelId grouping.
- [ ] Visual polish carried from Phase 3 (JetBrains Mono TTF, line icons, bg gradient/glows, glass blur).
