...
Date: 2026-07-05
Status: Approved direction (fully native Rust, Warp/Zed-style). This document is the master plan. **Phase 1 (pnpm migration) is COMPLETE; Phase 2 de-risk spike is COMPLETE; Phase 3 is COMPLETE (theme module, frameless titlebar, icon rail, toggleable status bar, styled pane frame, fixed-dark terminal scheme, column-view projection); Phase 4 backend + terminal-adjacent UI is COMPLETE (hand-built search/serialize/link engine, ported PTY lifecycle semantics, session persistence with 30s autosave, notifications model + panel, find bar, link underlines, copy-mode indicator, session-activity status dot). Remaining Phase 4 items (copy-mode selection/yank depth, animation polish) are tracked deferrals. Phase 5 (peripheral UI + integrations parity) is IN PROGRESS: the load-bearing pipe/UI single-writer unification (which closed the Phase 4 CLI/pipe->live-UI bridge deferral), the 264px session panel, the command palette + quick-launch + settings overlays, git/port status-bar pollers, and the native browser-method rejection are landed (versions 0.18.0-0.21.1); drag-split, agent manager, Claude-context/hooks/observer, markdown/diff surfaces, themes/i18n, and visual polish remain as tracked to-dos under the Phase 5 section.**
...
Delivered so far (2026-07-08, versions 0.18.0-0.21.1):
...
- [ ] Agent manager (`agent spawn`/`spawn-batch`/`status`/`list`/`kill`, round-robin pane distribution) + pandamux-orchestrator plugin verification (regression-test its scripts against the Rust pipe server). The orchestrator's visible-agent path depends on this + `layout grid` (already implemented).
- [ ] Claude Code integration: context injection, hooks config, activity observer (feeds the busy-agent status dot), sidebar status/progress/log (`set-status`/`set-progress`/`log`/`sidebar-state`).
- [ ] Markdown surface (evaluate Iced's `markdown` module) + diff surface, with the `markdown.set_content`/`markdown.load_file`/`diff.refresh` pipe methods and a content store (non-terminal surfaces currently render a placeholder).
