...
import { injectPandaMUXBlock } from '../../src/main/opencode-context';
import { pluginNeedsUpdate } from '../../src/main/opencode-context';

const START = '<!-- pandamux:start';
const END = '<!-- pandamux:end -->';
const BLOCK = `${START} v1 -->\nUse the pandamux CLI.\n${END}`;

describe('injectPandaMUXBlock', () => {
  it('returns the block alone when existing is empty', () => {
    expect(injectPandaMUXBlock('', BLOCK)).toBe(BLOCK);
  });
  it('appends the block when no markers present', () => {
    const out = injectPandaMUXBlock('# My rules\n', BLOCK);
  ...
  it('replaces an existing block, preserving surrounding content', () => {
    const old = `top\n${START} v0 -->\nOLD\n${END}\nbottom`;
    const out = injectPandaMUXBlock(old, BLOCK);
  ...
  it('is idempotent when the block source ends with a newline', () => {
    ...
    const blockNl = `${BLOCK}\n`;
    const first = injectPandaMUXBlock('', blockNl);
    const second = injectPandaMUXBlock(first, blockNl);
    expect(second).toBe(first);
  });
  it('is idempotent when appended after user content', () => {
    const blockNl = `${BLOCK}\n`;
    const first = injectPandaMUXBlock('# My rules\n', blockNl);
    const second = injectPandaMUXBlock(first, blockNl);
    expect(second).toBe(first);
  });
});
