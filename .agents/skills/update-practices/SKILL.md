...
model: opus
effort: high
description: Fetch latest Codex best practices and update the .Codex/ folder configuration. Safe to run repeatedly.
user-invocable: true
...
You have been asked to update this repository's Codex configuration to the latest best practices. Follow these steps exactly.
...
1. Read `.Codex/references/source-urls.md` to get the list of URLs to fetch. All external URLs used by this skill come from that registry; never hardcode URLs here.
2. Read `AGENTS.md` in the repo root. Note its contents and version references.
3. Read `agents.md` in the repo root. Note registered agents.
4. Scan `.Codex/skills/` using Glob. List all existing skills.
5. Scan `.Codex/agents/` using Glob. List all existing agents.
6. Read `.Codex/settings.json`. Note current settings.
7. Read `.Codex/references/tools.md` (if it exists). Note current tools.
8. Read `.Codex/references/design-guardrails.md` (if it exists). Note current guardrails.
9. Scan `.Codex/rules/` using Glob. List all existing path-scoped rules.
10. Scan `.Codex/agent-memory/` using Glob. List all existing memory files.

## Step 2: Fetch Latest Practices

Spin up parallel `explorer` agents to fetch and analyze sources. Use the custom `explorer` agent (defined in `.Codex/agents/`), never the built-in `Explore` type: the built-in loads every MCP tool schema and blows the context window.

1. **Official sources subagent:** "Fetch all official Anthropic sources from the source URL registry. Extract: current Codex version, new features, deprecated features, new recommended settings/skills/agents/hooks, folder structure changes, new frontmatter fields for agents and skills, new hook events, new settings options. WHY: We need to know what changed officially to update the config accurately."
...
3. **Stack freshness subagent:** "Check the project's detected stack (from AGENTS.md or dependency manifests) against current versions and best practices as of today's date. WHY: We need to ensure tools.md and design guardrails reflect the latest stable versions."
...
The `.Codex/` folder in this repo was originally scaffolded from the **Codex-bootstrap** template. This step checks the template for any new or updated files that should be pulled in.

The template repo URL, tree API URL, and raw-content base URL all live in the "Bootstrap Template" section of `.Codex/references/source-urls.md`. Read them from there.
...
1. Get the template's current main commit SHA cheaply, without downloading content:
   ...
   ```
2. Read `.Codex/references/template-sync-state.json` (if it exists). It stores `lastSyncedCommit`, `lastSyncedDate`, and `deadUrls` from the previous run.
...
The response contains every path in the template plus each file's git blob SHA. Extract all paths under `.Codex/`; these are the canonical template files. Ignore files outside `.Codex/` (AGENTS.md, README.md, package.json, etc.) since those are project-specific.
...
For each `.Codex/` path in the template tree:

1. **Check if it exists locally.** If not, check the ignore list (see below):
   - Listed in `.Codex/references/template-sync-ignore.md` → mark as **TEMPLATE-IGNORED** (deliberately removed by this project; do not re-create).
...
Only sync these categories of `.Codex/` files from the template:

| Category | Path pattern | Sync strategy |
|----------|-------------|---------------|
| Skills | `.Codex/skills/**` (SKILL.md plus supporting files under `references/`, `templates/`, `evals/`, etc.) | Sync new skills entirely, including their supporting files. For **TEMPLATE-UPDATED** files, merge new steps/sections but preserve project-specific customizations (e.g., custom matchers, stack-specific checks). For **TEMPLATE-REWRITTEN** files, **replace the body wholesale with the template version**, then re-apply only the genuinely project-specific additions (custom stack checks, custom paths). Do not try to merge a rewrite line-by-line. |
| Agents | `.Codex/agents/*.md` | Sync new agents entirely. For **TEMPLATE-UPDATED** agents, update frontmatter fields and instructions but preserve project-specific `context` or `skills` overrides. For **TEMPLATE-REWRITTEN** agents, replace the body with the template version, preserving only project-specific `context`/`skills`/`tools` overrides. |
| Rules | `.Codex/rules/*.md` | Sync new rules entirely. For **TEMPLATE-UPDATED** rules, update content but preserve custom `paths:` frontmatter if the project has different file structure. For **TEMPLATE-REWRITTEN** rules, replace the body with the template version, preserving only the project's custom `paths:` frontmatter. |
| Scripts | `.Codex/scripts/*.sh` | Sync new scripts entirely. For existing scripts, replace with template version unless local version has project-specific logic (check for project-specific paths, env vars, or tool references). |
| References | `.Codex/references/source-urls.md` | Merge: add any new URLs from template that are not already present. A URL may be removed only when it was verified unreachable on two consecutive runs (tracked via `deadUrls` in the sync state file). |
| References | `.Codex/references/hooks-and-settings.md` | Generic catalog, not project-specific. Replace with the template version unless the local copy has project-specific notes appended, in which case merge new rows in. |
| References | `.Codex/references/ux-laws.md` | Generic catalog, not project-specific. Replace with the template version unless the local copy has project-specific notes appended, in which case merge new entries in. |
| References | `.Codex/references/infrastructure.md` | Do NOT sync; infrastructure is project-specific. |
| References | `.Codex/references/tools.md` | Do NOT sync; tools depend on project stack. |
| References | `.Codex/references/design-guardrails.md` | Do NOT sync; guardrails depend on project stack. |
| Settings | `.Codex/settings.json` | Deep-merge: add new hooks, permissions, and env vars from template. Never remove existing entries. Preserve project-specific matchers and custom hooks. |
| Settings | `.Codex/settings.local.json.example` | Replace with template version (it is just an example file). |

### Files to NEVER sync

- `.Codex/agent-memory/*`: project-specific memory, never overwrite.
- `.Codex/references/template-sync-state.json`: generated locally by this skill.
- `.Codex/references/template-sync-ignore.md`: project-specific removal list; never overwrite local entries.
- Any file not in the `.Codex/` directory.
- `AGENTS.md`, `agents.md`, `instructions.md`: these are project-tailored.

### Apply template changes

For each **TEMPLATE-NEW** file:
- Create it locally with the template content.
- If the file previously existed and was deleted (visible in `git log`, or the user tells you), do not re-create it; add its path to `.Codex/references/template-sync-ignore.md` instead and report it as TEMPLATE-IGNORED.
...
After syncing, write `.Codex/references/template-sync-state.json`:
...
### Path-scoped rules (.Codex/rules/*.md)
...
### Agent memory (.Codex/agent-memory/)
...
The full event catalog, hook types, and matcher syntax live in `.Codex/references/hooks-and-settings.md`; check configured hooks against it. When the fetched sources reveal a hook event or type not yet in that reference, add it there (not into a skill body), so the catalog stays in one place.

### Settings
Check configured settings against the optional-settings catalog in `.Codex/references/hooks-and-settings.md`. That reference is the single source of truth for which settings exist; do not maintain a settings list inside this skill. If the latest Codex version introduces a new setting, add it to that reference.
...
**Canonical skill and agent inventory:** derive it from the template tree fetched in Step 2b (every `.Codex/skills/*/SKILL.md` and `.Codex/agents/*.md` path in the tree). Do not maintain a hardcoded list here; it drifts. Every template skill and agent should exist locally and be current unless it is listed in `template-sync-ignore.md`. If Step 2b was skipped because this repo is the template, the local tree IS the canonical inventory.

Update `.Codex/references/tools.md` if any tools have new versions or new tools should be added.

Update `.Codex/references/design-guardrails.md` if UI best practices have changed.
...
Review settings against the catalog in `.Codex/references/hooks-and-settings.md`, and ensure `settings.local.json.example` exists if it should.

Apply the two-strikes dead-URL rule to `.Codex/references/source-urls.md` using `deadUrls` from the sync state file (see Step 2b).

## Step 5: Prune AGENTS.md Files

Review all AGENTS.md files in the hierarchy. Remove:
- Advice the model now handles natively (check against current model capabilities)
- Outdated version references
- Redundant rules that duplicate parent AGENTS.md content

Keep each AGENTS.md focused and under 200 lines.

## Step 6: Prune and Validate Rules

Review `.Codex/rules/*.md` files:
- Remove rules that duplicate AGENTS.md content (rules should be path-specific, not general).
...
1. Update `AGENTS.md` if skill or agent inventory changed.
...
Determine the installed Codex version by running `Codex --version` locally; use web sources only as a fallback if the CLI is unavailable.
...
CHANGES APPLIED:
  ...
  [UPDATED] Modified .Codex/settings.json: <what changed>
...
FEATURES IN USE:
  - Path-scoped rules: <count> rules in .Codex/rules/
  - Agent memory: <count> files in .Codex/agent-memory/
