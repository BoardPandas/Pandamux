...
1. Read `.Codex/references/source-urls.md` to get the list of URLs to fetch.
...
```
WebFetch https://api.github.com/repos/BoardPandas/Codex-bootstrap/git/trees/main?recursive=1
```

Parse the JSON response. Extract all paths under `.Codex/` — these are the canonical template files. Ignore files outside `.Codex/` (AGENTS.md, README.md, package.json, etc.) since those are project-specific.
...
3. If it exists locally → fetch the template version:
   ```
   WebFetch https://raw.githubusercontent.com/BoardPandas/Codex-bootstrap/main/<path>
   ```
...
| References | `.Codex/references/source-urls.md` | Merge: add any new URLs from template that aren't already present. Never remove existing URLs. |
...
| Settings | `.Codex/settings.local.json.example` | Replace with template version (it's just an example file). |
...
### Settings
Check configured settings against the optional-settings catalog in `.Codex/references/hooks-and-settings.md` (`attribution.*`, `autoUpdatesChannel`, `sandbox.*`, `worktree.*`, `language`, `allowedHttpHookUrls`, `alwaysThinkingEnabled`, `disableAllHooks`). If the latest Codex version introduces a new setting, add it to that reference too.
...
BOOTSTRAP TEMPLATE SYNC:
  Template repo: BoardPandas/Codex-bootstrap
