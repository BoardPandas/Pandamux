...
model: opus
effort: high
description: Build or rebuild the .Codex/ folder with best practices. Use when setting up Codex in a new or existing repository. Run plan-repo first for new projects.
allowed-tools:
  - Read
  - Write
  - Edit
...
# Initialize Repository for Codex

You have been asked to initialize or upgrade this repository's Codex configuration. Follow these steps exactly.
...
1. Read `AGENTS.md` in the repo root (if it exists). Note its contents.
...
4. Scan the `.Codex/` folder (if it exists) using Glob. List all existing files.
5. Read `.Codex/settings.json` (if it exists). Note current settings.
6. Check for existing `.Codex/rules/*.md` files. Note any path-scoped rules already defined.
7. Check for existing `.Codex/agent-memory/` directory. Note any shared knowledge files.
...
3. Fetch each relevant concern's `llms.txt` index (e.g., `practices/Codex-config/llms.txt`, `practices/safety/llms.txt`).
...
- `Codex-config/hierarchical-Codex-md` -- structure AGENTS.md correctly
- `Codex-config/rule1-llg-integration` -- add RULE 1 to AGENTS.md
- `Codex-config/path-scoped-rules` -- create .Codex/rules/ files
- `Codex-config/hook-configuration` -- configure hooks
- `safety/credential-deny-list` -- add deny rules to settings.json
- `safety/read-only-first-rule` -- add RULE 0 to AGENTS.md
- `context-management/compact-and-handoff` -- add context management guidance
- `context-management/agent-memory` -- initialize .Codex/agent-memory/
...
4. Only note them at this point. Do not write them into any file yet; Step 14d seeds them into `.Codex/agent-memory/debugging.md` exactly once.

**3c. Fetch from web sources**

1. Read the source URL registry at `.Codex/references/source-urls.md`.
2. Spin up parallel `explorer` agents to fetch and analyze sources. Use the custom `explorer` agent (defined in `.Codex/agents/`), never the built-in `Explore` type -- the built-in loads every MCP tool schema and blows the context window.
   - **Subagent 1:** "Fetch official Anthropic sources and extract current Codex version, features, and recommended patterns. WHY: We need the latest official conventions to generate an up-to-date config."
...
2. Use WebSearch to find current best practices (as of today's date) for the detected stack:
   ...
   - Testing patterns
3. If the project has a UI, generate `.Codex/references/design-guardrails.md` with stack-specific UI/design SLA guidelines. Include:
...
Compare the current `.Codex/` folder against BP practices and web sources. For each applicable BP practice, run its CHECK items to see if the repo already follows it. Identify:
...
- Missing `.Codex/rules/*.md` files for path-scoped conventions (including bp-check.md and llg-check.md)
- Missing `.Codex/agent-memory/` directory for team knowledge
- Missing RULE 0 (read-only-first), RULE 1 (LL-G), RULE 3 (BP) in AGENTS.md
...
- **Template source:** The canonical template is the `BoardPandas/Codex-bootstrap` repository (the repo this `.Codex/` folder was copied from). Its AGENTS.md skill table and `agents.md` registry define the full template skill and agent lists. If a template skill, agent, rule, or script is missing here, copy it from that repo (via `git clone` to a temp directory or raw GitHub fetch) rather than reinventing it.
- **Non-destructive:** Never overwrite custom project-specific settings. Merge with existing config.
- **Skills:** Ensure all template skills exist in `.Codex/skills/`. If additional skills are relevant to the detected tech stack, add them.
- **Agents:** Ensure all template agents exist in `.Codex/agents/`. Add others if relevant.
- **Scripts:** Ensure `.Codex/scripts/` contains every shell script referenced by hooks in `settings.json` (e.g., `session-start-kb-check.sh`, the commit-check scripts). A hook that points at a missing script fails on every session start. Copy missing scripts from the template source.
- **Settings:** Update `.Codex/settings.json` with recommended permissions and hooks. Preserve existing custom entries.
- **Tools reference:** Update `.Codex/references/tools.md` with stack-specific CLI tools, install commands, and usage patterns. **Important:** There is no local Docker, no local Postgres, no local Redis -- all infrastructure runs remotely on Northflank and Cloudflare. Do not add local infrastructure tools (docker, docker-compose, psql, redis-cli). Preserve the existing **Available MCP Servers** section that documents all MCP integrations available to Codex.
- **AGENTS.md:** Build a hierarchical AGENTS.md structure:
  - Update root `AGENTS.md` with project-specific stack info, conventions, and skill/agent inventory.
  - Plan (but do not create) subfolder AGENTS.md files where distinct rules will apply.
  - Keep each AGENTS.md file focused and under 200 lines.
...
- **README.md:** If a README exists, add or update the "Codex" section. Do not alter other sections.

## Step 7: Configure Path-Scoped Rules

Create `.Codex/rules/` directory with conditional instruction files. Each rule file uses `paths:` frontmatter to load only when working with matching file patterns.

### How rules work
- Files in `.Codex/rules/*.md` have YAML frontmatter with a `paths:` array of glob patterns.
- A rule file only loads into context when Codex is working with files matching those patterns.
...
- `.Codex/rules/tests.md` — Testing conventions, paths: `["**/test/**", "**/*.test.*", "**/*.spec.*", "**/__tests__/**"]`
- `.Codex/rules/bp-check.md` — RULE 3 enforcement: check BP before modifying infrastructure/tooling configs. Paths: `["AGENTS.md", ".Codex/**", "Dockerfile*", "biome.*", "turbo.json", "vitest.config.*", "jest.config.*", ".github/**"]`
- `.Codex/rules/llg-check.md` — RULE 1 enforcement: check LL-G before writing code. Paths: source directories, e.g. `["src/**", "lib/**", "app/**", "worker/**", "api/**", "scripts/**", "middleware.*"]` (adjust to the detected project layout). Copy the template from the bootstrap repo's `.Codex/rules/llg-check.md`.

For frontend projects, also create:
- `.Codex/rules/frontend.md` — Component patterns, styling rules, accessibility. Paths matching frontend source dirs.
- `.Codex/rules/styles.md` — CSS/styling conventions. Paths matching style files.

For backend/API projects, also create:
- `.Codex/rules/api.md` — API conventions, error handling, auth patterns. Paths matching API source dirs.
- `.Codex/rules/database.md` — Migration rules, query patterns, ORM conventions. Paths matching schema/migration files.
...
Create `.Codex/agent-memory/` as a version-controlled team-shared knowledge base.

### Purpose
Agent memory is evolving knowledge that agents accumulate during work. Unlike AGENTS.md (static rules), agent memory captures discovered patterns, debugging insights, and project-specific knowledge that emerges over time.

### Structure to create
- `.Codex/agent-memory/README.md` — Explains the purpose and conventions for this directory.
- `.Codex/agent-memory/patterns.md` — Discovered code patterns and conventions (starts nearly empty).
- `.Codex/agent-memory/decisions.md` — Key technical decisions and their rationale (starts nearly empty).
- `.Codex/agent-memory/debugging.md` — Known gotchas and learning lessons. Initialize with this structure:
...
- Memory files complement AGENTS.md — AGENTS.md has rules, agent-memory has discovered knowledge.
...
- `context: <instructions>` — Additional context injected into the agent's system prompt. Use for agent-specific rules that don't belong in the main AGENTS.md.
...
- `memory: user|project|local` — Persistent cross-session memory scope. `user` = `~/.Codex/agent-memory/` (cross-project), `project` = `.Codex/agent-memory/` (team-shared), `local` = `.Codex/agent-memory-local/` (personal, git-ignored). First 200 lines of MEMORY.md are injected into the agent's system prompt on startup.

### Recommended agent enhancements

- **reviewer** agent: Add `memory: project` so it reads `.Codex/agent-memory/` on startup and reviews against discovered project patterns and decisions.
...
Read `.Codex/references/hooks-and-settings.md` for the full catalog: every hook event, the five hook types (command, http, prompt, agent, mcp_tool), the `if:` field, matcher syntax, and the "Hooks to configure based on project needs" recommendations. That file is the single source of truth — do not re-paste the event table into any skill or AGENTS.md.

For init, configure the always-on defaults from that reference (`SessionStart` knowledge-base reminder, `PreToolUse` `Bash(git commit*)`, `Stop` and `Notification` sounds). The always-on hooks invoke scripts in `.Codex/scripts/`; confirm those scripts exist (Step 6) before wiring the hooks, since a hook pointing at a missing script fails on every session start.
...
Update `.Codex/settings.json` with all relevant settings. Deep-merge with existing.

### Core settings (always configure)

The core-settings JSON block (permissions allow/deny lists, credential deny-list, `env`, `plansDirectory`) lives in `.Codex/references/hooks-and-settings.md` under "settings.json core settings". That file is the single source of truth; read it and apply it rather than working from memory. Do not re-paste the block into any skill, AGENTS.md, or instructions file.

### Optional settings and settings.local.json

The full optional-settings catalog (`attribution.*`, `autoUpdatesChannel`, `sandbox.*`, `worktree.*`, `language`, `allowedHttpHookUrls`, `alwaysThinkingEnabled`, `disableAllHooks`), the `settings.json` vs `settings.local.json` split, and the `.Codex/settings.local.json.example` template all live in `.Codex/references/hooks-and-settings.md`. Read it, then create `settings.local.json.example` from the template there.
...
- What the `.Codex/` folder contains
- How to use each skill (trigger phrase and description)
- Hierarchical AGENTS.md architecture explanation
- Path-scoped rules explanation (`.Codex/rules/*.md`)
- Agent memory explanation (`.Codex/agent-memory/`)
- Agent and skill frontmatter fields reference
- A pointer to `.Codex/references/hooks-and-settings.md` for hook events, hook types, matcher syntax, and the full settings catalog. Do not restate that content in instructions.md; link to it so there is one source of truth.
...
- [ ] Hierarchical AGENTS.md under 200 lines with tech stack, standards, workflow
- [ ] RULE 0 (read-only-first) present in AGENTS.md
- [ ] RULE 1 (LL-G integration) present in AGENTS.md with 4-step protocol
- [ ] RULE 3 (BP integration) present in AGENTS.md
- [ ] Credential deny-list in settings.json
- [ ] Context management guidance (compact at 50%, handoff docs)
- [ ] Plans must end with lessons learned stated in AGENTS.md
- [ ] Plan-then-execute workflow documented
- [ ] Anti-pattern awareness documented
- [ ] `.Codex/rules/bp-check.md` exists
- [ ] `.Codex/rules/llg-check.md` exists
...
Write `.Codex/bp-audit.md` with full results:
```markdown
# BP Audit Results
Date: <YYYY-MM-DD>
...
If LL-G had HIGH-severity entries matching this project's tech stack (from Step 3b), add a summary to `.Codex/agent-memory/debugging.md`:
```markdown
