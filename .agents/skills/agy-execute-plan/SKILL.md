...
You orchestrate a handoff: an existing plan (written earlier by Codex, usually in `tasks/`) is executed end-to-end by the Antigravity CLI (`agy`), and then you verify that the plan was actually carried out, repair what AGY got wrong, and report anything that blocked either AGY or you.
...
5. **Detect the project commands** you will need in Step 3: the test command, the build/typecheck command, and the lint command. Read `package.json` scripts (or the equivalent for the stack) and the project's AGENTS.md. If you cannot find them, note that and plan to verify by other means.
...
2. **Standards and lint.** Run the project linter/formatter check and confirm the changes follow the project's coding standards (AGENTS.md, rules/). Note violations.
