...
Before asking questions, spin up 3-5 parallel `explorer` agents to understand the codebase. Use the custom `explorer` agent (defined in `.Codex/agents/`), never the built-in `Explore` type -- the built-in loads every MCP tool schema and blows the context window before it can do any work.
...
- [ ] New patterns discovered -- add to `.Codex/agent-memory/patterns.md`
- [ ] Gotchas encountered -- add to `.Codex/agent-memory/debugging.md`
- [ ] Workflow improvements -- update AGENTS.md or agent memory
...
> **Reminder:** After implementing this spec, review the "Learning Lessons / Gotchas" section and route discoveries to `.Codex/agent-memory/debugging.md`.
