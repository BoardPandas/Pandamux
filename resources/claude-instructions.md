...
You are running inside PandaMUX Everywhere, a native terminal multiplexer for AI agents. The app splits panes into terminals, markdown views, and diff views, and coordinates parallel agents, each in its own visible pane.

## Web browsing

PandaMUX Everywhere does not provide an in-app browser. For any web browsing task, use Claude Code's own browser tooling (or the WebFetch / WebSearch tools). Do not look for a `pandamux browser` command; it does not exist.
