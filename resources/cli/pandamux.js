...
function readPipeToken() {
    ...
    try {
        ...
        return fs_1.default.readFileSync(path_1.default.join(base, `pandamux${suffix}`, 'pipe-token'), 'utf-8').trim();
    }
    catch {
        return '';
    }
    ...
    try {
        switch (command) {
            ...
            case 'set-color-scheme': {
                ...
                if (!surfaceId) {
                    console.error('No surface id. Pass one as argument or run inside a pandamux pane.');
                    process.exit(1);
                }
                if (!scheme) {
                    console.error('Usage: pandamux set-color-scheme [surfaceId] <scheme>');
                    process.exit(1);
                }
            ...
            case 'config': {
                ...
                if (sub === 'show' || sub === 'get') {
                    ...
                    console.error('Usage: pandamux config <show|reload|path>');
                    process.exit(1);
                }
                break;
            }
            ...
            case 'agent': {
                ...
                switch (sub) {
                    ...
                    case 'spawn-batch': {
                        ...
                        if (jsonIdx === -1) {
                            console.error('Usage: pandamux agent spawn-batch --json \'[...]\'');
                            process.exit(1);
                        }
            ...
            case 'agent-activity': {
                const surfaceId = getFlag(args, '--surface') || process.env.PANDAMUX_SURFACE_ID;
                if (!surfaceId) {
                    console.error('agent-activity: --surface or PANDAMUX_SURFACE_ID required');
                    process.exit(1);
                }
