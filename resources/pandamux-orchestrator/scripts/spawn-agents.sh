...
# Creates a balanced grid of pandamux panes (orchestrator + N agents) and spawns
# one Claude Code agent per new pane. Uses `pandamux layout grid` so all panes are
...
# binaries spawned via `pandamux agent spawn --cmd` can resolve them. No-op on Linux/macOS.
winpath() {
...
PANDAMUX_AVAILABLE=false
if command -v pandamux &>/dev/null; then
  PING_RESULT=$(pandamux ping 2>&1)
  if [ "$PING_RESULT" = "pong" ]; then
    PANDAMUX_AVAILABLE=true
  else
    echo "WARNING: pandamux found but ping failed: $PING_RESULT" >&2
  fi
else
  echo "WARNING: pandamux not found in PATH" >&2
fi

if [ "$PANDAMUX_AVAILABLE" != "true" ]; then
  echo "pandamux unavailable — writing pending spawn file for degraded mode" >&2
...
# in row-major order. The CLI picks up $PANDAMUX_SURFACE_ID as anchor automatically.
GRID_COUNT=$((AGENT_COUNT + 1))
echo "Creating $GRID_COUNT-cell grid layout (1 orchestrator + $AGENT_COUNT agents)"

LAYOUT_RESULT=$(pandamux layout grid --count "$GRID_COUNT" --type terminal 2>&1)
FIRST_PANE=$(parse_json "$LAYOUT_RESULT" '.newPaneIds[0]')
if [ -z "$FIRST_PANE" ] || [ "$FIRST_PANE" = "null" ]; then
  echo "ERROR: pandamux layout grid failed: $LAYOUT_RESULT" >&2
  exit 1
fi
...
while IFS= read -r agent; do
  ...
  SPAWN_RESULT=$(pandamux agent spawn \
