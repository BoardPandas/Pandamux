#!/usr/bin/env node
/**
 * dashboard-text.js — ANSI-art dashboard for pandamux-orchestrator runs.
 * Used in two places:
 *   1. Fallback mode (no pandamux): the orchestrate skill calls this at wave
 *      transitions to print a status block into Claude Code's conversation.
 *   2. pandamux mode: can be called manually by the user to get a one-shot view.
 *
...
// ─── Header ─────────────────────────────────
const titleLeft = `${c.bold(c.amber('pandamux'))} ${c.faint('•')} ${c.amber('orchestration')}`;
