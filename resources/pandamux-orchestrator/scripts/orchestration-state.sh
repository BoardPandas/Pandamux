#!/usr/bin/env bash
# orchestration-state.sh — State management library for pandamux orchestrations.
...
find_active_orch() {
  local latest=""
  local latest_time=0
  for dir in "$ORCH_BASE"/pandamux-orch-*/; do
    [ -d "$dir" ] || continue
...
get_orch_dir() {
  local id="$1"
  echo "$ORCH_BASE/pandamux-orch-$id"
}

# Create a new orchestration directory
create_orch_dir() {
  local id="$1"
  local dir="$ORCH_BASE/pandamux-orch-$id"
  mkdir -p "$dir"
  echo "$dir"
}
