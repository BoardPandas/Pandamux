...
# It does NOT fire for agents spawned via `pandamux agent spawn` (those are independent
# processes). In pandamux mode, the orchestrate SKILL.md drives wave transitions by
# polling `pandamux agent list` instead.
  ...
  if all_waves_done "$ORCH_DIR"; then
    ...
    if [ "$REVIEWER_STATUS" = "pending" ]; then
      ...
      if command -v pandamux &>/dev/null; then
        pandamux notify "All agents complete. Starting reviewer..." 2>/dev/null || true
      fi
    else
