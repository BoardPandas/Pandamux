...
> "pandamux-orchestrator plugin not found. Install it with: `claude plugins install pandamux-orchestrator@pandamux-orchestrator`"
...
Store the result as `PANDAMUX_MODE`:
- If output is `"available"` → `PANDAMUX_MODE=pandamux` — agents spawn in visible terminal panes via `pandamux agent spawn`
- If output is `"unavailable"` → `PANDAMUX_MODE=degraded` — agents spawn as invisible Claude Code subagents via the Agent tool
...
```bash
mkdir -p "${TMPDIR:-/tmp}/pandamux-orch-$ORCH_ID"
```
...
```json
{
      ...
      "agents": [
        {
          ...
          "exitCode": null,
          "toolUses": 0,
          "resultFile": "/tmp/pandamux-orch-XXXXXX/agent-a-result.md",
          "startedAt": null,
          "finishedAt": null
        }
      ]
  ...
  "reviewer": {
    "status": "pending",
    "agentId": null,
    "reportFile": "/tmp/pandamux-orch-XXXXXX/review-report.md"
  }
}
```
...
The spawn script (`spawn-agents.sh`) automatically creates panes via `pandamux split`.
...
This script:
1. Creates a pane per agent via `pandamux split`
...
3. If there are more waves:
   ...
   c. Verify agents spawned with `pandamux agent list`
