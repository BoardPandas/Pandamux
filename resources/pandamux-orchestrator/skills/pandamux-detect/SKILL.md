...
description: Detect if PandaMUX Everywhere terminal multiplexer is running. Used internally by orchestrate skill to decide between PandaMUX Everywhere multi-pane mode and degraded subagent mode.
---

# PandaMUX Everywhere Detection
...
Run the detection script to check if PandaMUX Everywhere is available:
...
- PandaMUX Everywhere is running and the named pipe is accessible
...
- PandaMUX Everywhere is not running or not installed
...
- Log: "PandaMUX Everywhere not detected. Running in degraded mode — agents will use Claude Code's native subagent system. Install PandaMUX Everywhere for the full multi-pane experience: https://pandamux.boardpandas.ai"
...
**ENFORCEMENT:**
- When `PANDAMUX_AVAILABLE=true`: ALL agents MUST be spawned via `pandamux agent spawn`. Do NOT use Claude Code's `Agent` tool. The Agent tool creates invisible subagents — the user chose PandaMUX Everywhere specifically to SEE agents in panes.
