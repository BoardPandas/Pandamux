...
resources/        Runtime assets (icons, themes, sounds, shell-integration, CLI)
  pandamux-orchestrator/  Codex plugin (auto-installed on startup)
...
| `Codex-context.ts` | Auto-injects PandaMUX Everywhere instructions into `~/.Codex/AGENTS.md`, configures hooks, installs pandamux-orchestrator plugin |
| `Codex-observer.ts` | Monitors Codex activity for sidebar display |
...
For new Codex integrations, add CLI commands in `src/cli/pandamux.ts`.
...
Codex plugin bundled in `resources/pandamux-orchestrator/`. Auto-installed into `~/.Codex/plugins/cache/` on startup by `ensureOrchestratorPlugin()` in `Codex-context.ts`. Also published standalone: `github.com/amirlehmam/wmux-orchestrator`.

**What it does:** Decomposes complex dev tasks into parallel Codex agents coordinated through dependency-aware waves with automated review. With PandaMUX Everywhere: each agent in its own visible terminal pane. Without PandaMUX Everywhere: falls back to native subagents.

**Plugin structure:**
```
resources/pandamux-orchestrator/
  .Codex-plugin/plugin.json    Manifest (name, version, author)
  ...
  scripts/spawn-agents.sh       Creates panes + launches Codex agents
...
Meta:    metadata:update, hook:event, Codex:activity
```
...
- **No MCP**: All Codex integration via CLI commands
- **Workflow source of truth (`.Codex/`)**: before committing, update `CHANGELOG.md` and bump the version per `.Codex/rules/commit-changelog.md`; write commit messages to a file and use `git commit -F` (never inline `-m`). Consult the BP and LL-G knowledge bases before config/code work (`.Codex/rules/bp-check.md`, `.Codex/rules/llg-check.md`). Use the custom agents in `.Codex/agents/`, not built-in subagent types.
