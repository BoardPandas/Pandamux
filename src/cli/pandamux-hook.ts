#!/usr/bin/env node
/**
 * pandamux hook helper — sends a hook event to the pandamux pipe.
 * Called by Claude Code hooks (PostToolUse, Notification, Stop).
 *
 * Usage:
 *   node pandamux-hook.js <tool-name>        # PostToolUse — sidebar/diff tracking
 *   node pandamux-hook.js --event <Event>    # Notification / Stop — fires a pandamux notification
 *
 ...
 * PANDAMUX_SURFACE_ID (set by pandamux in each pane's shell) ties the event to its pane.
 */
import net from 'net';
...
const pipePath = process.env.PANDAMUX_PIPE || '\\\\.\\pipe\\pandamux';
const token = process.env.PANDAMUX_PIPE_TOKEN || '';
const surfaceId = process.env.PANDAMUX_SURFACE_ID || '';
...
function sendHook(): void {
  ...
  client.on('error', () => {
    // pandamux not running — silently ignore.
    process.exit(0);
  });
}
